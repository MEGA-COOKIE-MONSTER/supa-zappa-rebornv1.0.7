...
var bossLaserImg, bossImg, bossLaser,boss, bossReqPoints, bossReqNum,heartEarnGroup,heartImg
var bossReqPointsArray=[]
var r =0
var soundPlaying
// to preload assets
function preload(){
    ...
    bossLaserImg= loadImage("assets/boss-laser.png")
...
function draw(){
    ...
    if(gamestate == "play"){
        ...
        if(frameCount % 600 == 0){
            var heartEarn = createSprite(random(50,625), random(50,605), 50,50)
            heartEarn.addImage(heartImg)
            heartEarn.scale = 0.35
            heartEarn.lifetime = 100
            heartEarnGroup.add(heartEarn)
        }
        ...
        if(player1.isTouching(heartEarnGroup)){
            powerupSound.play()
            lives++
            heartEarnGroup.destroyEach()
        }
        ...
        if(score >= bossReqNum){
            ...
            bossReqPoints=Math.round(bossReqPoints + (bossReqPoints/2))
            bossReqPointsArray.push(bossReqPoints)
            
            gamestate == "boss"
            for(i = 0; i< bossReqPointsArray.length ; i++){
                bossReqNum += bossReqPointsArray[i]
            }
        }
        console.log(bossReqNum, bossReqPoints)
    }
    if(gamestate == "boss"){
